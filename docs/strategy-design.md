...
3. 世界状态枚举：ω ∈ {主胜,平,客胜}³（≤27 种），P(ω) = ∏ p_{i,r_i}
4. 整数线性规划：
   - **主目标**：max P_eff(x) = ∑_ω P(ω) · 𝟙[Payoff(ω,x) ∈ [900,2700]]
   - **次目标**：在主目标最大解集中，max N_cov(x) = |{ω : P(ω)>0 ∧ Payoff(ω,x)∈[900,2700]}|
   - 约束：∑x_c ∈ [146,154]，∑x_c (c∈A) ≤ 30，格点约束，用户勾选软约束
5. 用户勾选软约束：每个用户勾选 r 必须有 c 覆盖且 x_c≥2，**除非**满足以下之一：
   - 覆盖 r 的所有 c 都被窗口剔除
   - 强制保留 r 会让 P_eff 下降 > 5%
   舍弃时必须在报告中明示理由
6. 自救：用户组合不可行时 AI 在 30 元预算内枚举更多 A 候选；仍不可行则 fail 并提示用户调整
7. 输出：每注金额/赔率/概率，P_eff，N_cov，E[收益] vs E_naive，被舍弃的勾选+理由
