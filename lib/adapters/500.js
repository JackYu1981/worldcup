...
export class Adapter500 extends BaseAdapter {
  ...
  buildKaijiangUrl(date) {
    // 注意：500.com 这里的参数名是 d 不是 date；?date= 会被忽略并回落到"当前期次"，
    // 导致跨期次（延期补赛、历史回扫）抓不到对应那一天的开奖。
    return `https://zx.500.com/jczq/kaijiang.php?d=${date}`;
  }
  ...
  transformMatch(rowHtml) {
    ...
    // 必须是纯数字才视为有效 fixture id。
    // 守卫历史：早期只挡空字符串，但 500.com 过渡窗口会写 data-fixtureid="-1"
    // 之类的占位值，绕过 `!fixtureId` 守卫导致写出 id="f-1" 的幽灵副本
    // （参见 2026-06-21 西班牙-沙特重复 bug）。
    ...
    // period 用 500.com 的 data-processdate（"期次日期"，权威字段，与彩票销售期次一一对应）
    // 不要用 buyendtime/kickoff 推导 —— 它们和 period 没有固定关系
    const period = getAttr('processdate') || null;
